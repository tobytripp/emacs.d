...
** ADOPTED EShell ‘visual’ commands
   - State "ADOPTED"    from "REJECTED"   [2017-12-08 Fri 11:30]
...
*** Returning from a 'visual' command

    Visual commands run in a new term-mode buffer named *env<n>*, killing this
    buffer will not, by default, return you to the shell that started it.  This
    function will kill the current buffer and open the eshell.

    #+BEGIN_SRC emacs-lisp
      (defun toby/kill-buffer-and-open-eshell ()
        (interactive)
        (toby/kill-this-buffer)
        (eshell))

      (global-set-key (kbd "C-c q") 'toby/kill-buffer-and-open-eshell)
    #+END_SRC
...
* Comint Mode
  ...
  #+BEGIN_SRC emacs-lisp
    (defun set-my-shell-bindings ()
      ...
      (local-set-key (kbd "C-c k")  'toby/kill-this-buffer)
      (local-set-key (kbd "C-c q")  'toby/kill-this-buffer)
