...
** Navigation
   ...
   #+BEGIN_SRC emacs-lisp
     ...
     (global-set-key (kbd "<end>")    'ace-window)
     (global-set-key (kbd "<select>") 'ace-window)
     (global-set-key (kbd "<next>")   'ido-switch-buffer)
...
** Frame/Window Properties
   #+BEGIN_SRC emacs-lisp
     (define-key global-map (kbd "C-c s +") 'text-scale-increase)
     (define-key global-map (kbd "C-c s -") 'text-scale-decrease)
     (define-key global-map (kbd "C-c s =") 'text-scale-set)
...
** File and Buffer Handling
   ...
   A function for `killing` the current buffer window without prompting:

   #+BEGIN_SRC emacs-lisp
     (defun toby/kill-this-buffer ()
       (interactive)
       (kill-buffer (current-buffer)))
   #+END_SRC

   And key-bindings to call it:

   #+BEGIN_SRC emacs-lisp
     (global-set-key (kbd "C-c k") 'toby/kill-this-buffer)
   #+END_SRC
